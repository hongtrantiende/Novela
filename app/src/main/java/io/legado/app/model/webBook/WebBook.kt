...
object WebBook {
    ...
    suspend fun runPreUpdateJs(bookSource: BookSource, book: Book): Result<Unit> {
        return kotlin.runCatching {
            ...
            AppLog.put("Không thực thi được quy tắc preUpdateJs Nguồn sách: ${bookSource.bookSourceName}", it)
        }
    }
    ...
    suspend fun getContentAwait(
        ...
        if (bookSource.getContentRule().content.isNullOrEmpty()) {
            Debug.log(bookSource.bookSourceUrl, "⇒Nội dung văn bản trống, sử dụng liên kết chương: ${bookChapter.url}")
            return bookChapter.url
        }
        if (bookChapter.isVolume && bookChapter.url.startsWith(bookChapter.title)) {
            Debug.log(bookSource.bookSourceUrl, "⇒Văn bản thư mục cấp độ không phân tích các quy tắc")
    ...
    fun preciseSearch(
        ...
        return Coroutine.async(scope, context, semaphore = semaphore) {
            ...
            throw NoStackTraceException("<$name>$author không được tìm thấy")
        }
    }
    ...
    suspend fun preciseSearchAwait(
        ...
        return kotlin.runCatching {
            ...
            throw NoStackTraceException("Không tìm thấy sách của $name($author)")
        }.onFailure {
            coroutineContext.ensureActive()
        }
    }
