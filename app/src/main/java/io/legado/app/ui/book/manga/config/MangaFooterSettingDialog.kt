...
class MangaFooterSettingDialog :
        ...
        MangaScrollMode.ALL.forEach { mode ->
            binding.chipGroupScrollMode.addView(Chip(requireContext()).apply {
                ...
                setOnLongClickListener {
                    ...
                    toastOnUi("Đặt làm chế độ mặc định chung")
                    true
                }
            })
        }
    ...
    private fun updateChapterText() {
        ...
        val label = if (!config.hideChapterLabel) "chương" else ""
        val chapter = if (!config.hideChapter) "1/45 " else ""
        val name = if (!config.hideChapterName) "Chương 3" else ""

        val progressLabel = if (!config.hideProgressRatioLabel) "tổng tiến độ" else ""
        val progress = if (!config.hideProgressRatio) "2.1%" else ""

        val pageLabel = if (!config.hidePageNumberLabel) "Số trang" else ""
