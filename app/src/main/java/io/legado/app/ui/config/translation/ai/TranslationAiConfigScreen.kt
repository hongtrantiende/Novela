...
@Composable
fun TranslationAiConfigRouteScreen(
    onBackClick: () -> Unit,
    viewModel: TranslationAiConfigViewModel = koinViewModel()
) {
    TranslationAiConfigScreen(
...
@Composable
fun TranslationAiConfigScreen(
    state: TranslationAiConfigUiState,
    effects: Flow<TranslationAiConfigEffect>,
    onIntent: (TranslationAiConfigIntent) -> Unit,
    onBackClick: () -> Unit
) {
    ...
    LaunchedEffect(Unit) {
        effects.collectLatest { effect ->
            when (effect) {
                is TranslationAiConfigEffect.ShowMessage -> {
                    snackbarHostState.showSnackbar(effect.message)
                }
                is TranslationAiConfigEffect.NavigateBack -> {
                    onBackClick()
                }
            }
        }
    ...
    AppScaffold(
        ...
        floatingActionButton = {
            AppFloatingActionButton(
                onClick = { onIntent(TranslationAiConfigIntent.Save) },
            ...
            item {
                SplicedColumnGroup(title = stringResource(R.string.ai_prompt_setting)) {
                    ClickableSettingItem(
                        ...
                        onClick = {
                            onIntent(TranslationAiConfigIntent.OpenPromptDialog(state.promptTemplate))
                        }
                    )
                    ClickableSettingItem(
                        ...
                        onClick = {
                            onIntent(TranslationAiConfigIntent.ResetPrompt)
                        }
                    )
                }
            }
            item {
                SplicedColumnGroup(title = stringResource(R.string.ai_param_setting)) {
                    SliderSettingItem(
                        ...
                        onValueChange = { onIntent(TranslationAiConfigIntent.UpdateTemperature(it)) }
                    )
                    InputSettingItem(
                        ...
                        onConfirm = { input ->
                            ...
                            onIntent(TranslationAiConfigIntent.UpdateMaxOutputTokens(tokens))
                        }
                    )
                }
            }
    ...
    val editPromptDialog = activeDialog as? TranslationAiConfigDialog.EditPrompt
    AppAlertDialog(
        show = editPromptDialog != null,
        onDismissRequest = { onIntent(TranslationAiConfigIntent.CloseDialog) },
        title = stringResource(R.string.ai_custom_prompt),
        confirmText = stringResource(R.string.confirm),
        onConfirm = {
            if (editPromptDialog != null) {
                onIntent(TranslationAiConfigIntent.UpdatePrompt(editPromptDialog.currentPrompt))
            }
        },
        dismissText = stringResource(R.string.cancel),
        onDismiss = { onIntent(TranslationAiConfigIntent.CloseDialog) },
        content = {
            if (editPromptDialog != null) {
                AppTextField(
                    value = editPromptDialog.currentPrompt,
                    onValueChange = { onIntent(TranslationAiConfigIntent.UpdateDialogPrompt(it)) },
                    singleLine = false,
                    maxLines = 10,
                    modifier = Modifier
                        .fillMaxWidth()
