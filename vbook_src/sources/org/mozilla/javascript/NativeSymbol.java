...
public class NativeSymbol extends ScriptableObject implements Symbol {
    ...
    public static void init(Context context, Scriptable scriptable, boolean z) {
        LambdaConstructor lambdaConstructor = new LambdaConstructor(scriptable, CLASS_NAME, 0, 1, new zx7(8));
        lambdaConstructor.setPrototypePropertyAttributes(7);
        lambdaConstructor.defineConstructorMethod(scriptable, "for", 1, new by7(lambdaConstructor, 3), 2, 3);
        lambdaConstructor.defineConstructorMethod(scriptable, "keyFor", 1, new ay7(28), 2, 3);
        lambdaConstructor.definePrototypeMethod(scriptable, "toString", 0, new ay7(29), 2, 3);
        lambdaConstructor.definePrototypeMethod(scriptable, "valueOf", 0, new ly7(0), 2, 3);
        SymbolKey symbolKey = SymbolKey.TO_PRIMITIVE;
        lambdaConstructor.definePrototypeMethod(scriptable, symbolKey, 1, new ly7(0), 3, 3);
        SymbolKey symbolKey2 = SymbolKey.TO_STRING_TAG;
        lambdaConstructor.definePrototypeProperty(symbolKey2, CLASS_NAME, 3);
        lambdaConstructor.definePrototypeProperty(context, "description", new f1(7), 3);
    ...
    private static Object js_for(final Context context, final Scriptable scriptable, Object[] objArr, final LambdaConstructor lambdaConstructor) {
        final String scriptRuntime;
        if (objArr.length > 0) {
            scriptRuntime = ScriptRuntime.toString(objArr[0]);
        } else {
            scriptRuntime = ScriptRuntime.toString(Undefined.instance);
        }
        return getGlobalMap(scriptable).computeIfAbsent(scriptRuntime, new java.util.function.Function() { // from class: my7
