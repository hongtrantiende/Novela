package defpackage;
/* compiled from: r8-map-id-f559a6c9210f67746bd9ce87931ee85b676f3bda66e04dc5e85cb7e8eec71de1 */
/* renamed from: edf  reason: default package */
/* loaded from: classes.dex */
public final class edf extends m7e {
    private static final edf zzc;
    private static volatile k8e zzd;
    private int zze;
    private ogf zzf;

    /* JADX WARN: Type inference failed for: r0v0, types: [edf, m7e] */
    static {
        ?? m7eVar = new m7e();
        zzc = m7eVar;
        m7e.j(edf.class, m7eVar);
    }

    public static ddf t() {
        return (ddf) zzc.o();
    }

    public static /* synthetic */ void u(edf edfVar, ogf ogfVar) {
        ogfVar.getClass();
        edfVar.zzf = ogfVar;
        edfVar.zze |= 1;
    }

    public static edf v() {
        return zzc;
    }

    /* JADX WARN: Type inference failed for: r2v12, types: [k8e, java.lang.Object] */
    @Override // defpackage.m7e
    public final Object d(int i) {
        k8e k8eVar;
        switch (gdf.a[i - 1]) {
            case 1:
                return new m7e();
            case 2:
                return new l7e(zzc);
            case 3:
                return new n8e(zzc, "\u0000\u0001\u0000\u0001\u0002\u0002\u0001\u0000\u0000\u0000\u0002ဉ\u0000", new Object[]{"zze", "zzf"});
            case 4:
                return zzc;
            case 5:
                k8e k8eVar2 = zzd;
                if (k8eVar2 == null) {
                    synchronized (edf.class) {
                        try {
                            k8e k8eVar3 = zzd;
                            k8eVar = k8eVar3;
                            if (k8eVar3 == null) {
                                ?? obj = new Object();
                                zzd = obj;
                                k8eVar = obj;
                            }
                        } catch (Throwable th) {
                            throw th;
                        }
                    }
                    return k8eVar;
                }
                return k8eVar2;
            case 6:
                return (byte) 1;
            default:
                throw null;
        }
    }

    public final ogf w() {
        ogf ogfVar = this.zzf;
        if (ogfVar == null) {
            return ogf.y();
        }
        return ogfVar;
    }
}
